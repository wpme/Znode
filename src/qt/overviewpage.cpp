...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::ZNDE)
    {
    }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("ZNDE")
    updateDisplayUnit();
}
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeZNDECoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeZNDECoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeZNDECoinAmount = strAnonymizeZNDECoinAmount.remove(strAnonymizeZNDECoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeZNDECoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeZNDECoinAmount * COIN) nMaxToAnonymize = nAnonymizeZNDECoinAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeZNDECoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeZNDECoinAmount));
        strAnonymizeZNDECoinAmount = strAnonymizeZNDECoinAmount.remove(strAnonymizeZNDECoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeZNDECoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
